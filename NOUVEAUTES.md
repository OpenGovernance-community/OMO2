...
- Les participants a un scrutin public disposent maintenant d un bouton de deconnexion. Il revoque leur lien personnel recu par e-mail et renvoie vers la demande d acces, y compris apres un retour arriere du navigateur.
- Les parametres generaux des scrutins permettent maintenant de rendre les resultats intermediaires accessibles ou non a l organisateur. Ce choix est affiche dans les options visibles par les participants.
...
- Les dossiers NextCloud existants restent consultables lorsque leur reponse WebDAV ne fournit pas d identifiant de fichier : leur chemin reste prioritaire, et l identifiant ne sert de secours que lorsque ce chemin ne permet plus de retrouver le dossier.
- Les événements associés à un projet disposent maintenant d un menu pour les modifier, les supprimer ou les dupliquer. La duplication ouvre uniquement un nouvel éditeur prérempli, conserve notamment les invités et le lieu, mais laisse les dates vides tant que le nouvel événement n est pas enregistré.
- Les PDF issus dun dossier NextCloud utilisent maintenant le meme cadre de previsualisation en pleine hauteur que les fichiers televerses; les documents Collabora distants reprennent aussi leur cadre standard.
- Les Documents peuvent maintenant contenir un dossier NextCloud reference par son chemin ou choisi dans larborescence distante. Son contenu est relu a chaque ouverture, applique les droits du dossier OMO et permet la previsualisation PDF, le telechargement et louverture Collabora des fichiers compatibles. Leur identifiant NextCloud est conserve et permet de retrouver puis mettre a jour le chemin apres un renommage ou un deplacement dans le dossier configure.
- Les fichiers PDF televerses sont maintenant affiches directement dans le detail du document, sans carte ni titre interieurs. Leur URL d apercu porte le nom du fichier, ce que les lecteurs PDF de navigateur affichent correctement; les boutons Telecharger et Plein ecran sont disponibles cote a cote.
- L ouverture directe dun document associe a un projet ouvre maintenant systematiquement son detail, y compris lorsque ce document nest pas affiche dans la liste courante ou que le drawer Documents etait deja charge.
- Chaque projet dispose maintenant de sa discussion partagee, y compris lorsqu il est encore en proposition. Toute personne qui peut voir le projet peut y ecrire, sans devoir en etre responsable. Le detail affiche le total de messages dans son entete et, apres une intervention personnelle, le nombre de messages recus depuis celle-ci, sans compter ses propres messages ni afficher un compteur nul. Les actions de l entete restent alignees en haut.
- Les reglages de notifications des Projets permettent maintenant de choisir les canaux pour les commentaires sur ses projets ou propositions et pour les discussions auxquelles on participe. Les notifications ouvrent directement le projet concerne.
...
- Un timer mobile, concu comme une app plein ecran compacte, permet maintenant de choisir une organisation dans une liste verticale, puis un holon et un projet par trois vues navigables au swipe tactile, directement attachees a chaque panneau et tolerantes aux gestes diagonaux courts, y compris depuis la carte des holons. Son conteneur principal, ainsi que la carte des holons, sont bord a bord et sans libelle redondant ; les noms d organisations trop longs sont tronques sans defilement horizontal et la marque de la topbar suit l organisation choisie. Le picker partage affiche maintenant, comme la mini-navigation OMO, les libelles des holons contenus dans la cible plutot que son propre libelle, y compris apres son survol. Sa bulle courante reste visible, avec son libelle seulement lorsqu il s agit d un role, et les seuils de libelles sont abaisses a 9 px sur telephone. Un selecteur affiche par defaut les projets en cours, avec les statuts Pret, Bloque et A verifier disponibles ; leurs priorites utilisent les couleurs partagees, sont placees a gauche, trient la liste de P1 a P5 et leurs titres sont limites a deux lignes. Le panneau Projets reste vide avant la selection d un holon. Le retour qui demande de choisir un holon disparait des cette selection et toute erreur s efface automatiquement apres dix secondes. Le timer lance ou arrete le suivi du temps, change de cible sans perdre la ligne precedente, sauvegarde son avancement toutes les 30 secondes et reprend un suivi encore ouvert apres une interruption du telephone ou du navigateur.
...
- La liste Projets precharge aussi les holons et responsables distincts en deux lots. Les fiches Kanban, liste et Gantt ne relisent donc plus un holon et un utilisateur par projet; les responsables recoivent uniquement leurs quatre champs d affichage.
- Les listes Documents, Calendrier, Decisions, Regles, Statistiques, Equipe et Projets remplacent maintenant leur contenu par un squelette scintillant pendant les rechargements distants declenches par les filtres ou la portee. La liste Documents calcule aussi l activite des dossiers et la presence d enfants en memoire apres une seule lecture de metadonnees, au lieu de refaire une requete par dossier et par document supprimable. Les evenements lies aux PV sont egalement charges par lot.
...
- Les vues des applications suivent maintenant trois niveaux persistants : chaque utilisateur garde sa vue par holon dans `user_holon`, l admin d organisation definit un defaut pour le modele directement herite, et le super admin definit les valeurs globales par type de holon. L effacement d une vue personnelle ou de modele laisse automatiquement place au niveau precedent.
- La fleche accolee a Enregistrer forme maintenant un groupe compact sans arrondi interieur. Son menu rassemble les enregistrements, puis les restaurations et suppressions de defauts sous un separateur.
- Dans l editeur du tableau de pilotage, les actions Enregistrer et Restaurer sont maintenant regroupees : la fleche du bouton Enregistrer ouvre un seul menu, avec les sauvegardes puis les restaurations separees visuellement.
...
- Chaque fiche du tableau de pilotage peut maintenant etre configuree individuellement. Sa portee Local, Enfants directs ou Descendants est enregistree dans la disposition et s affiche discretement dans son en-tete; les portees qui ne sont pas pertinentes pour un holon, notamment un role sans enfant, reviennent automatiquement a Local. Le filtre global et sa recherche rapide ont ete retires du tableau.
- Les sauvegardes du tableau distinguent maintenant les preferences personnelles en Expert, les vues propres aux holons, les templates directement herites et les quatre types de base Organisation, Cercle, Role et Groupe. Les admins d organisation gerent ces valeurs pour leur organisation; le super admin peut en plus definir le defaut global et les variantes globales par type, sans tenter de partager les templates propres a une organisation.
- Le bouton de retour aux valeurs par defaut du tableau peut maintenant effacer la vue personnelle, puis via sa fleche les valeurs par defaut du holon, du template direct ou des types de base autorises. Le defaut global pour toutes les organisations reste volontairement protege de cette suppression.
- Les actions de sauvegarde, de suppression et de fermeture de l editeur du tableau sont maintenant compactes et les deux actions principales peuvent tenir cote a cote sans etirer excessivement son en-tete.
- La fiche Evenements compte maintenant les rendez-vous planifies sur les trente prochains jours, pour tous puis pour l utilisateur courant. Ses compteurs basculent la liste sans rechargement, et une liste simple ou double reste bornee avec defilement lorsqu elle contient davantage d elements.
...
- Dans Team, le menu d un membre permet maintenant d editer son affectation : focus, budget temps et budget argent, avec une recurrence par jour, semaine, mois ou annee, ainsi qu une date limite de reconfirmation ou d arret. Les budgets sont affiches l un sous l autre dans les fiches de membres, tandis que la popup aligne Focus et date limite sur une ligne. Dans les fiches, la date limite reste sous la date d affectation et la colonne des dates laisse davantage de place aux mois longs. Les affectations en retard sont reperees par un rouge discret en fiches et en vue compacte. Le bloc Focus conserve ses retours a la ligne sans en ajouter avant sa valeur. La popup reutilise directement le cadre de la modale pour eviter une bordure interne et un defilement superflu.
- Dans le panneau de structure, les vignettes des membres d un role affichent maintenant une ligne Focus dans leur infobulle lorsqu il est renseigne.
...
- Les resultats du questionnaire proposent maintenant trois axes OMO a fort potentiel. Pour chacun des dix principes, le parcours associe des capacites concretes d OMO a cinq niveaux et n affiche que les etapes situees entre le niveau actuel et le niveau vise. Le cinquieme niveau garde un rappel de vigilance.
- Dans le questionnaire, le passage d Aujourd hui a Demain devient visible meme apres un choix bas dans la liste : la page remonte les deux reperes et active clairement Demain. Sur ecran large, ces reperes restent visibles pendant la lecture des propositions.
- Le questionnaire de maturite peut maintenant etre enregistre. Il genere un lien public court limite a la situation actuelle et un lien prive qui ouvre directement le rapport final ; le bouton de relecture permet ensuite de modifier les reponses avant de sauvegarder le meme resultat. Sur un resultat deja enregistre, Sauver les modifications reste desactive tant qu aucune reponse ne change, tandis que Obtenir mes liens reste disponible. Un resultat peut aussi etre associe a un compte puis a une organisation OMO avec le flux de connexion par code e-mail existant. Les reponses sont stockees une par principe et par score, ce qui prepare les moyennes et comparaisons entre membres.
- La vue publique ajoute la legende des dix principes a cote du radar. Chaque element de cette legende ouvre directement le detail correspondant plus bas dans la page. Le radar et les details montrent maintenant la situation actuelle et la situation souhaitee afin de rendre l ambition visible ; les reperes Aujourd hui et Demain sont places sous le radar.
- Le survol ou le focus d un principe dans la legende publique met en evidence ses deux points Aujourd hui et Demain sur le radar, tout en attenuant les autres reperes.
- Apres l association d un resultat de maturite a une organisation OMO, la page revient directement au rapport prive. L organisation deja associee est preselectionnee lors d une nouvelle ouverture.
- Le questionnaire peut maintenant etre envoye aux membres, holons et parties prenantes externes d une organisation. Chaque lien recu fixe l organisation evaluee, cree ou retrouve le compte associe a l e-mail lors de la sauvegarde, et permet ensuite au repondant de mettre a jour sa propre perception sans devenir membre de l organisation. Les reponses liees a ces invitations ne produisent pas de lien public.
- Seuls les admins d une organisation peuvent maintenant lancer une campagne d evaluation : les autres organisations ne sont pas proposees dans le selecteur et l API applique la meme verification.
- Une premiere evaluation ouverte depuis une invitation est maintenant enregistree automatiquement des l affichage du rapport final. Si le reseau empeche cet enregistrement, les reponses restent sur l appareil et le bouton de sauvegarde reste disponible.
- Dans le rapport du questionnaire, les actions de sauvegarde et de relecture sont placees directement apres le radar ; les pistes OMO et le redemarrage restent a la fin des resultats.
- Une page publique par organisation est disponible dans `/survey/organization.php?id=...`. Elle utilise la banniere, le logo et la couleur de l organisation, puis affiche les moyennes Aujourd hui et Demain, la participation des membres et la dispersion de l affinite pour chaque principe.
- La presentation publique compacte maintenant l identite de l organisation au-dessus du radar, avec la participation, le graphique et sa legende dans une seule carte visible des le premier ecran sur ordinateur. Les compteurs de membres et d evaluations ne sont plus affiches.
- Le radar de cette carte utilise maintenant presque toute la surface disponible : son rayon est augmente, il est aligne en haut et la legende passe sur deux colonnes afin de supprimer le grand vide vertical provoque par sa hauteur. Seule la legende conserve un fond discret pour rester lisible sur la banniere.
- La page publique d organisation adopte une carte d identite complete inspiree de la maquette : bandeau avec logo et participation, radar et synthese dans une carte a gauche, puis liste illustree et cliquable des dix dimensions a droite. La courbe bleue pointillee represente Aujourd hui et la zone turquoise Demain ; les fiches detaillees restent disponibles sous la carte.
...
- Le chargement de la structure OMO est maintenant asynchrone et partage entre la mini-carte, le panneau complet et les selecteurs de portee. La representation reste en cache pendant toute la session tant que l historique de l organisation ne change pas, et sa construction regroupe les holons, proprietes et membres en quelques requetes au lieu de les charger noeud par noeud.
...
- Les refus de modification CalDAV exposes sur beta indiquent maintenant le compte et les holons effectivement evalues, afin de diagnostiquer un droit ou une identite incoherente sans consulter les logs serveur.
- Un lien CalDAV cible publie maintenant deux agendas avec le meme filtre de holons : `Modifiables` pour les evenements autorises par `CAN_EDIT_EVENT` et `Lecture seule` pour les autres evenements visibles.
...
- Les noms des projets archives sont maintenant des liens vers leur sous-drawer de detail via le hash projects-d{id}. Le detail reste consultable en lecture seule, avec ses documents et evenements visibles sans actions de creation.
...
- Les evenements associes a un projet sont regroupes comme dans la vue liste du calendrier : aujourd hui, demain, cette semaine, la semaine prochaine, puis les mois suivants.
...
- Le detail d'un projet garde ses onglets visibles, sans espace residuel en bas, avec un contenu defilant et un espacement generique propre a chaque onglet.
...
- L import OMO1 ne reutilise plus la premiere autorite d un holon pour tous les domaines suivants lorsque leurs libelles ne correspondent pas. Chaque domaine distinct conserve ainsi son propre identifiant et son propre libelle.
- Les autorites locales des templates importes depuis OMO1 sont maintenant resolues vers une copie propre a chaque holon utilisateur. Les regles ciblent cette copie liee au template, et les listes d autorites heritees acceptent aussi les valeurs agregees de plusieurs niveaux afin que getOrg affiche les identifiants locaux.
- L import OMO1 des regles exporte maintenant aussi les domaines disponibles. Sans modele, ou avec un modele qui utilise des proprietes de type autorite, les domaines sont crees comme autorites et les regles y sont rattachees. Les parenthes disponibles sont reprises automatiquement lorsqu elles sont non ambigues; les autorites sans parent sont signalees en rouge pour un rattachement manuel.
- La propriete OMO1 des domaines est maintenant exportee et importee comme une liste d autorites, avec conversion des valeurs vers les autorites creees. Les modeles de base des roles n affichent plus la propriete Strategie.
- Les groupes sont maintenant transparents pour la recherche des autorites parentes : l editeur, la validation des proprietes et l import remontent jusqu au premier cercle contenant.
- Les autorites deja rattachees a une propriete restent affichees meme lorsqu aucune nouvelle autorite ne peut etre creee; le bouton d ajout est alors retire et le controle serveur reste actif.
- Le marquage interne des autorites OMO 1 sans parent ne rajoute plus de texte parasite dans leur description.
- Les regles echues sont maintenant signalees en rouge comme obsoletes, et celles dont la date de revue est atteinte en jaune comme a verifier.
- Le cache de la feuille de styles OMO est invalide apres l ajout des marquages visuels des regles, afin que les couleurs apparaissent immediatement.
- Le marquage des regles utilise maintenant les variables du composant generique omo-card avec une priorite suffisante pour conserver le fond et la bordure d alerte.
- Les regles modifiables disposent maintenant d un menu « ... » ouvrant leur formulaire d edition dans le drawer habituel.
- L edition des regles utilise maintenant l editeur HTML simple pour le gras, l italique, les listes et les liens. Le mode regle filtre aussi les tableaux, images et elements complexes colles, tandis que Rule applique la meme protection lors des sauvegardes et des imports, sans changer le format HTML des exports simples.
- L intention des regles est maintenant facultative: l import OMO1 conserve celle qui existe et laisse le champ a NULL lorsqu elle est absente, sans ajouter de texte generique.
- Les regles peuvent maintenant etre affichees sans regroupement, dans une liste plate; ce mode se combine avec le tri par date de creation ou de modification pour reperer rapidement les regles recentes, anciennes ou delaissees.
- Les autorites locales definies dans un modele de holon sont maintenant instanciees sur les holons utilisant ce modele. Leur libelle et leur description restent synchronises; les copies sont en italique et non editables. Une suppression de l autorite source conserve les copies en signalant la perte de leur origine.
- Le mode administrateur d organisation donne aussi l acces general d edition aux holons et modeles, utilise notamment lors de la suppression ou de la modification de leurs proprietes.
- Le retrait d une propriete dans un editeur de modele est maintenant transmis explicitement a la sauvegarde, y compris lorsque plusieurs proprietes ont le meme libelle.
- Les autorites du holon Organisation peuvent maintenant servir de parent aux autorites des cercles de premier niveau, meme a travers des groupes transparents.
- La creation d une organisation depuis un modele instancie aussi les autorites locales des templates sur leurs holons structurels. Le formulaire de regles rattrape les instances manquantes sur les structures deja creees.
- La sauvegarde d une autorite locale dans un modele relit maintenant la valeur qui vient d etre enregistree : les ajouts et modifications sont bien appliques a l autorite source puis a toutes les instances de cercle existantes.
- Les exports OMO2 incluent maintenant les autorites et les regles. L import recree les hierarchies d autorite, les liens vers leurs sources de modele, les proprietes qui les referencent et les regles locales ou rattachees a un domaine.
- L import OMO1 choisit maintenant le traitement des domaines par holon. Sans modele applique, les domaines portes par un template deviennent des autorites locales puis sont instancies sur ses roles structurels; les domaines ordinaires se rattachent a l unique autorite disponible du parent quand cela est possible, sinon restent visibles en rouge. Avec un modele applique, le format du modele est conserve: un domaine n est lie a une autorite que si le holon cible propose une liste d autorites et qu une correspondance non ambigue est trouvee; sinon ses regles restent locales au holon.
- L export OMO1 suit maintenant les roles source et cercles source references par les roles structurels, y compris les identifiants resolus par la traduction de la forme de gouvernance OMO1. Ces roles internes a l application ne deviennent jamais des holons exportes: leur definition complete et leurs heritages sont materialises dans les templates et roles structurels de l organisation. Les attendus et domaines propres a une instance restent sur cette instance et ne sont donc plus recopies sur les autres roles structurels. Les domaines communs sont rattaches au template exporte pour pouvoir devenir des autorites locales a l import.
- L exporteur OMO1 expose temporairement une revision de generateur et un diagnostic des sources structurelles dans les metadonnees techniques du fichier. Cela permet d identifier le stockage historique reel des attendus sans exporter les roles internes de l application comme holons.
- La definition des roles structurels OMO1 est maintenant lue directement par les objets de l application: le type de gouvernance est traduit par Organisation, puis les attendus et domaines visibles sont obtenus par les methodes du role source. Le fichier 3.php sert de repli lorsque la traduction de l organisation est indisponible.
- Les codes structurels 2, 4, 8 et 16 ne sont plus traites comme des identifiants de roles ou de types SQL. Ils servent uniquement de cles de traduction vers les vrais roles source de la gouvernance.
- L export OMO1 reste compatible avec les anciennes versions de PHP lors du traitement du POST: les appels de compatibilite n utilisent plus de fonctions ou types recents et les parcours des objets sources sont proteges lorsque les listes sont absentes.
- Un POST d export sans case cochee ne relance plus l export Structure par defaut: le formulaire affiche correctement le message demandant de selectionner au moins un element.

## 2026-07-29

- Les ecrans de creation et d edition des applications OMO partagent maintenant les memes espacements de drawer, sections de formulaire, libelles, grilles et rangees d actions. Le rendu automatique adminEdit, les editeurs Calendrier, Documents, Projets, Checklists, Regles, Indicateurs, Decisions, Holons et LMS suivent ainsi une mise en page commune.
- Les titres, descriptions, informations secondaires, libelles et textes d aide d OMO reposent maintenant sur un socle typographique partage, avec des densites distinctes pour les fiches et les listes compactes. Documents, Projets, Decisions, Calendrier, Team, LMS, les checklists, les templates, les indicateurs et les popups reutilisent ces variantes au lieu de redefinir localement leur couleur, taille, graisse et interligne.
- Le socle visuel OMO partage maintenant une echelle globale pour les espacements, retraits, surfaces et rayons. Les popups, formulaires de parametres, templates de holons et cartes de propositions reutilisent les memes conteneurs generiques, sans modifier sensiblement leur mise en page.
- Les etats vides de Reglement et Indicateurs reprennent maintenant le panneau accentue des Decisions, avec un retrait genere par le nouveau composant partage generic-empty-hero.
- Le panneau des Indicateurs distingue maintenant ses attributs internes des boutons de filtre, ce qui retablit les choix de classement et de representation.
- La liste des Checklists reprend maintenant la presentation compacte des Documents, avec un retrait lateral base sur les variables generiques de liste.
- Checklists et Decisions utilisent maintenant le filtre compact partage, avec recherche rapide, application temporaire et vue enregistree par contexte; les etats des Decisions restent visibles sous le champ, et Type et Methode se choisissent en capsules.
- Le menu de filtres du Calendrier passe au-dessus de ses entetes fixes, et le champ partage est plus compact avec une zone de capsules plus legere.
- Calendrier, Indicateurs et Documents utilisent maintenant le filtre compact partage: selections en capsules, recherche rapide, panneau de choix, application temporaire pour la session et enregistrement de la vue par contexte.
- Les filtres compacts de Projets, TEAM et Regles partagent maintenant un champ plus arrondi, avec une zone de selections grise et des capsules blanches legerement ombrees.
- TEAM dispose maintenant du meme filtre compact que Projets: le perimetre et la representation apparaissent en capsules, une recherche rapide filtre les membres, et la vue peut etre appliquee temporairement pour la session ou enregistree pour le contexte courant.
- Les fiches des membres de Team utilisent maintenant ce terme plutot que Cartes, remplissent l espace photo avec les initiales et affichent separement les connexions a l organisation, au site et la date de creation, avec des mois localises.
...
- L export et l import OMO1 prennent maintenant en charge les regles: elles sont choisissables dans les formulaires, rattachees au bon holon a la migration et visibles dans l application Reglement. Les regles de domaine restent temporairement locales au holon porteur, dans l attente de l import des autorites; les dates de validite absentes d OMO1 recoivent des valeurs techniques signalees dans les avertissements.
- Le rattachement des regles OMO1 utilise maintenant l identifiant de holon exporte, distinct de l identifiant de role source. Les avertissements restent affiches cinq secondes avant la redirection apres une importation.
- L import OMO1 conserve les logos, icones et bannieres seulement lorsque leur fichier image est bien present sur le serveur courant; les references absentes ou externes sont remplacees par une valeur vide et signalees dans les avertissements. La fenetre d import affiche maintenant un ecran d attente avec indicateur graphique pendant le traitement.
...
- L icone effective d un holon est maintenant affichee devant son nom dans le panneau de contexte lorsqu elle est definie, dans un cadre blanc de 38 pixels avec une bordure de 2 pixels qui reste lisible sur les themes sombres; le fil d ariane precede maintenant le type de holon et son titre.
...
- L export d organisation accepte aussi les admins sans activation du mode admin.
...
- 2026-07-29 : Les entetes des groupes de Reglement occupent toute la largeur, tandis que les regles restent lisibles sur une surface blanche sans cadre ni libelle de description redondant.
- 2026-07-29 : Les fiches de Reglement mettent davantage en avant leur titre et leur enonce ; l intention et les metadonnees sont regroupees dans Informations et tracabilite.
- 2026-07-29 : Le libelle Intention des Reglements est a nouveau traduit correctement, et l enonce reprend la typographie generique de l interface.
- 2026-07-30 : La suppression d une organisation nettoie maintenant les regles et domaines d autorite importes avant les holons, et l API conserve le motif d un echec de suppression.
- 2026-07-30 : La collecte des holons descendants utilise maintenant des parametres SQL distincts pour chaque occurrence du filtre, afin que la suppression couvre bien toute la structure importee avec PDO MySQL.
