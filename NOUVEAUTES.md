...
Dans l edition d un scrutin, cocher seulement `Participation sans invitation` suffit maintenant a faire basculer le bloc `Invites` vers le resume avec ses boutons, meme sans invitation nominative. Et si cette ouverture publique rencontre une ancienne ligne participant inutilisable pour un utilisateur existant, une nouvelle ligne active peut a nouveau etre creee a partir de son `IDuser` au lieu de bloquer d office l acces.

L ouverture publique par e-mail reutilise maintenant aussi une ancienne ligne `decision_participant` inactive ou revoquee quand elle correspond deja a un membre de l organisation ou a une adresse connue de ce scrutin. Cela evite les echecs de creation lies aux contraintes uniques quand ces personnes redemandent un acces.
