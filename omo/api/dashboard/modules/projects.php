...
<div class="omo-personal-space__item-list omo-dashboard-module__list">
    <?php foreach ($dashboardProjectItems as $projectItem): ?>
        <button type="button" class="omo-personal-space__item-button omo-dashboard-project-item omo-dashboard-project-item--<?= omoApiEscape($projectItem['status']) ?>" data-omo-dashboard-filter-item="<?= omoApiEscape(implode(' ', $projectItem['filters'])) ?>" data-omo-personal-space-project-id="<?= (int)$projectItem['id'] ?>" data-omo-personal-space-project-holon-id="<?= (int)$projectItem['holonId'] ?>">
