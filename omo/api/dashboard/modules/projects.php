...
<div class="omo-personal-space__item-list omo-dashboard-module__list">
    <?php foreach (array_slice($dashboardProjectItems, 0, 20) as $projectItem): ?>
        <button type="button" class="omo-personal-space__item-button" data-omo-dashboard-filter-item="<?= omoApiEscape(implode(' ', $projectItem['filters'])) ?>" data-omo-personal-space-project-id="<?= (int)$projectItem['id'] ?>" data-omo-personal-space-project-holon-id="<?= (int)$projectItem['holonId'] ?>"<?= $dashboardModuleForcedOpenScope !== '' ? ' data-omo-personal-space-project-scope="' . omoApiEscape($dashboardModuleForcedOpenScope) . '"' : '' ?>>
    ...
    <?php if (count($dashboardProjectItems) > 20): ?><p class="omo-dashboard-module__more"><?= omoApiEscape(t('personal_space.module.more', ['count' => count($dashboardProjectItems) - 20], $lang, $sourceLang)) ?></p><?php endif; ?>
</div>
