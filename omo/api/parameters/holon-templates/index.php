...
} elseif (empty(($discoveryModeAccess = omoHolonTemplateDiscoveryModeAccess($organization))['status'])) {
                    ...
                    <div id="omo-template-form-content"<?= $showTemplateWelcome ? ' hidden' : '' ?>>
                        ...
                        <section class="omo-template-section generic-section generic-section--stack generic-section--roomy omo-template-section--permissions">
                            <div class="omo-template-permissions__sticky-navigation">
                                ...
                                <div class="omo-template-permissions__tabs" id="omo-template-permissions-tabs" role="tablist"></div>
                            </div>
                        ...
                        <section class="omo-template-section generic-section generic-section--stack generic-section--roomy">
                            <div class="omo-template-section__head">
                                <div>
                                    <div class="omo-template-section__title generic-title generic-title--subsection"><?= htmlspecialchars(omoHolonTemplateT('parameters.holon_templates.section.public_share'), ENT_QUOTES, 'UTF-8') ?></div>
                                    <p class="omo-template-section__description generic-description">
                            ...
                            <div class="omo-template-form__grid">
                                <label class="omo-template-flags__option generic-soft-panel generic-stack generic-stack--compact omo-field--full">
...
<style>
.omo-template-permissions__tabs {
    display: flex;
    gap: 0.5rem;
...
.omo-template-permissions__tab.is-active {
    border-bottom-color: var(--color-primary, #2563eb);
    color: var(--color-primary, #2563eb);
}

.omo-template-permissions__panel[hidden] {
...
const omoHolonTemplateElements = {
    ...
    permissionsTabs: omoHolonTemplateRoot.querySelector('#omo-template-permissions-tabs'),
...
function omoHolonTemplateRenderPermissions(permissionAssignments) {
    ...
    if (!permissionCatalog.length) {
        omoHolonTemplateElements.permissionsTabs.innerHTML = '';
    ...
    profiles.forEach(function (profile, index) {
        tabsHtml += '<button type="button" class="omo-template-permissions__tab' + (index === 0 ? ' is-active' : '') + '" data-permission-profile-tab="' + profile.key + '" role="tab" aria-selected="' + (index === 0 ? 'true' : 'false') + '">' + omoHolonTemplateEscapeHtml(profile.label) + '</button>';
    });
    let html = '';

    profiles.forEach(function (profile, profileIndex) {
        html += '<div class="omo-template-permissions__panel" data-permission-profile-panel="' + profile.key + '" role="tabpanel"' + (profileIndex === 0 ? '' : ' hidden') + '>';
    ...
    omoHolonTemplateElements.permissionsTabs.innerHTML = tabsHtml;
    omoHolonTemplateElements.permissions.innerHTML = html;
    omoHolonTemplateSyncPermissionGroupStickyOffset();

    Array.from(omoHolonTemplateElements.permissionsTabs.querySelectorAll('[data-permission-profile-tab]')).forEach(function (tab) {
        tab.addEventListener('click', function () {
            const profileKey = String(tab.getAttribute('data-permission-profile-tab') || 'member');
            Array.from(omoHolonTemplateElements.permissionsTabs.querySelectorAll('[data-permission-profile-tab]')).forEach(function (otherTab) {
                const isActive = otherTab === tab;
                otherTab.classList.toggle('is-active', isActive);
                otherTab.setAttribute('aria-selected', isActive ? 'true' : 'false');
            });
            Array.from(omoHolonTemplateElements.permissions.querySelectorAll('[data-permission-profile-panel]')).forEach(function (panel) {
...
.omo-settings__nested-drawer .omo-template-editor {
    height: 100%;
    min-height: 0;
}

.omo-settings__nested-drawer .omo-template-editor > .omo-panel-view__body {
    display: flex;
    min-height: 0;
    height: 100%;
    box-sizing: border-box;
...
.omo-template-editor__layout {
    display: grid;
    grid-template-columns: minmax(240px, var(--omo-template-sidebar-width, 340px)) 10px minmax(320px, 1fr);
    gap: 0;
    align-items: stretch;
    min-height: 0;
}

.omo-settings__nested-drawer .omo-template-editor__layout {
    flex: 1 1 auto;
    height: 100%;
}
...
.omo-template-editor__layout--holon-definition .omo-template-editor__resize-handle,
.omo-template-editor__layout--compact .omo-template-editor__resize-handle {
    display: none;
}

.omo-template-editor__resize-handle {
    position: relative;
    z-index: 3;
    min-height: 100%;
    touch-action: none;
...
.omo-template-editor__layout.is-resizing,
.omo-template-editor__layout.is-resizing * {
...
.omo-settings__nested-drawer .omo-template-sidebar,
.omo-settings__nested-drawer .omo-template-form-panel {
    overflow: auto;
}

.omo-template-sidebar__top {
    position: sticky;
    top: 0;
    z-index: 2;
    display: grid;
...
.omo-template-section {
    --generic-section-border: color-mix(in srgb, var(--color-border) 86%, transparent);
    --generic-section-background: color-mix(in srgb, var(--color-surface) 90%, var(--color-surface-alt));
}

.omo-template-section--permissions {
    position: relative;
}

.omo-template-permissions__sticky-navigation {
    position: sticky;
    top: -16px;
    z-index: 10;
    margin: -16px -16px 0;
...
.omo-template-permissions__sticky-navigation .omo-template-permissions__tabs {
    margin: 12px 0 0;
}

.omo-template-permissions__tabs:empty {
    display: none;
}

.omo-template-permissions__group {
    display: grid;
    gap: 0.75rem;
}

.omo-template-permissions__group + .omo-template-permissions__group {
    margin-top: 1rem;
    padding-top: 1rem;
    border-top: 1px solid color-mix(in srgb, var(--color-border) 86%, transparent);
}

.omo-template-permissions__group-title {
    position: sticky;
    top: var(--omo-template-permission-group-sticky-top, 0px);
    z-index: 5;
    margin: 0;
...
.omo-template-section__description {
    margin-top: var(--generic-space-1);
}
...
.omo-template-admin-bounds__fields {
    display: grid;
    grid-template-columns: repeat(2, minmax(0, 1fr));
    gap: 12px;
}

.omo-template-admin-bounds__lock {
    display: inline-flex;
    align-items: center;
    gap: 7px;
    margin-top: 8px;
...
.omo-template-admin-bounds__lock input {
    width: 15px;
    height: 15px;
    margin: 0;
    accent-color: var(--color-primary);
...
.omo-field input:not([type="checkbox"]):not([type="radio"]),
.omo-field select,
.omo-field textarea {
    display: block;
    width: 100%;
...
.omo-field input:not([type="checkbox"]):not([type="radio"]):focus,
...
.omo-template-property {
    ...
    gap: 12px;
    align-items: start;
    --generic-section-background: color-mix(in srgb, var(--color-surface) 76%, var(--color-surface-alt));
}
...
.omo-template-authority__row.is-needs-parent {
...
.omo-template-authority__local-label {
    display: flex;
    align-items: center;
    gap: 7px;
    color: var(--color-text-light);
...
.omo-template-welcome[hidden],
#omo-template-form-content[hidden] {
    display: none !important;
}

.omo-template-welcome__content {
    display: grid;
    gap: 14px;
    max-width: 620px;
    text-align: center;
...
.omo-template-welcome__content p {
    margin: 0;
    color: var(--color-text-light);
    line-height: 1.5;
}

.omo-template-welcome__content .omo-button {
    justify-self: center;
}
